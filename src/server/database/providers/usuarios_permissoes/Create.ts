...
export const create = async (usuario_permissao: Omit<IUsuarioPermissao, 'id'>): Promise<void | Error> => {
  try {
    ...
    await Knex(ETableNames.usuario_permissao)
      .insert({
        organizacao_id: usuario_permissao.organizacao_id,
        empresa_id: usuario_permissao.empresa_id,
