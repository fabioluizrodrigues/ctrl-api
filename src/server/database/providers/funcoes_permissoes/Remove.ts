...
export const remove = async (funcao_permissao: Omit<IFuncaoPermissao, 'id'>): Promise<void | Error> => {
  try {
    ...
    await Knex(ETableNames.funcao_permissao)
      .where('organizacao_id', '=', funcao_permissao.organizacao_id)
      .where('empresa_id', '=', funcao_permissao.empresa_id)
      .where('funcao_id', '=', funcao_permissao.funcao_id)
      .where('permissao_id', '=', funcao_permissao.permissao_id)
      .del();
