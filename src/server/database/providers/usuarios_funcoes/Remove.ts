...
export const remove = async (usuario_funcao: Omit<IUsuarioFuncao, 'id'>): Promise<void | Error> => {
  try {
    ...
    await Knex(ETableNames.usuario_funcao)
      .where('organizacao_id', '=', usuario_funcao.organizacao_id)
      .where('empresa_id', '=', usuario_funcao.empresa_id)
      .where('usuario_id', '=', usuario_funcao.usuario_id)
      .where('funcao_id', '=', usuario_funcao.funcao_id)
      .del();
